...
def get_all_have_pic_to_aprove():
    collection = db[pta]
    all_have_pic = collection.distinct('_id')
    return all_have_pic
...
def fetch_image_to_aprove(matrícula: str) -> Image.Image:
    col_pix = db[pta]
    std_pic = col_pix.find_one({'_id': matrícula})
    if std_pic:
        return std_pic['pic'], std_pic['thumb']
    else:
        return None
...
def main(user, logout):
    ...
    st.title("AVALIAR FOTOS ENVIADAS PELO ESTUDANTE")


    all_students = com_pic_and_repic
    ...
    if result:
        ...
        if received is None:
            ...
            try:
                pic, thumb = fetch_image_to_aprove(result[2])
            except TypeError:
                return None
            st.write('principal')
            st.markdown(f'<img src="data:image/jpg;base64,{pic}" />', unsafe_allow_html=True)
            st.success(str(len(f'~<img src="data:image/jpg;base64,{pic}" />')//1024) + ' KB')
            st.write('miniatura')
            st.markdown(f'<img src="data:image/jpg;base64,{thumb}" />', unsafe_allow_html=True)
            st.success(str(len(f'~<img src="data:image/jpg;base64,{thumb}" />')//1024) + ' KB')
            if st.button('Enviar foto para o banco de dados e apagar a foto em aprovação'):
                upload_pic_and_thumb(result[2], pic, thumb)
                st.success('Foto enviada com sucesso!')
                collection = db[pta]
                collection.delete_one({'_id': result[2]})
